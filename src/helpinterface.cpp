...
HelpInterface::HelpInterface(QWidget *parent) : QWidget(parent), ui(new Ui::HelpInterface) {
    ...
    move(1157, 100);
    setVisible(false);
    ui->HelpInterfaceLabel->setProperty("mode","HelpInterfaceLabel1");
    ui->Title_Label->setText("血压传感器可能异常 ");
    ui->Title_Label->setProperty("mode","30px,colour255,255,255");

    ui->Content_Label->setText("当传感器或者传感器电缆中任意导线断开或与其他导线短路会引起不正常的状态时，或当传感器的连接线被拔出时触发。传感器输出端子之间的短路检测不做要求。");
